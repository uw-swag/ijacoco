...
import org.ijacoco.core.rts.data.BinStorer;
import org.ijacoco.core.rts.data.DependencyAnalyzer;
import org.ijacoco.core.rts.data.PrefixTxtStorer;
import org.ijacoco.core.rts.data.Storer;
import org.ijacoco.core.rts.data.TxtStorer;
import org.ijacoco.core.rts.hash.FileCachingHasher;
import org.ijacoco.core.rts.hash.Hasher;
import org.ijacoco.core.rts.hash.Hasher.Algorithm;
...
public final class Config {
	...
	public static String RTS_DIR_V = rtsDirDefault();
	protected static final String RTS_DIR_N = "rts.dir";

	private static String rtsDirDefault() {
		return System.getProperty("user.dir")
				+ System.getProperty("file.separator")
				+ Names.RTS_ROOT_DIR_NAME;
	}

	/**
	 * Returns a File that describes .rts directory. Note that the directory is
	 * not created with this invocation.
	 *
	 * @param parentDir
	 *            Parent directory for .rts directory
	 * @return File that describes .rts directory
	 */
	public static File createRootDir(File parentDir) {
		return new File(parentDir, Names.RTS_ROOT_DIR_NAME);
	}

	/**
	 * Returns absolute path to .rts director (URI.toString). Note that the
	 * directory is not created with this invocation.
	 *
	 * @param parentDir
	 *            Parent directory for .rts directory
	 * @return An absolute path (URI.toString) that describes .rts directory
	 */
	public static String getRootDirURI(File parentDir) {
		String pathAsString = parentDir.getAbsolutePath()
				+ System.getProperty("file.separator")
				+ Names.RTS_ROOT_DIR_NAME;
	...
	public static enum AgentMode {
		NONE, SINGLE, SINGLEFORK, MULTI, JUNIT, JUNITFORK, SCALATEST;
	...
	private static Algorithm HASH_ALGORITHM_V = Hasher.Algorithm.CRC32;
	...
	public static final String RUN_INFO_V = createFileNameInCoverageDir(
			Names.RUN_INFO_FILE_NAME);

	@Opt(desc = "Enable/disable time logging.")
	public static boolean X_TIME_LOG_V = true;
	protected static String X_TIME_LOG_N = "x.log.time";

	/** Name of the directory to keep time logging */
	public static final String TIME_LOG_V = createFileNameInCoverageDir(
	...
	public static void loadConfig(String options, boolean force) {
		...
		File userHomeDir = new File(userHome, Names.RTS_CONFIG_FILE);
		Properties homeProperties = getProperties(userHomeDir);
		File userDir = new File(System.getProperty("user.dir"),
				Names.RTS_CONFIG_FILE);
		...
		Log.init(
				DEBUG_MODE_V == DebugMode.SCREEN
						|| DEBUG_MODE_V == DebugMode.EVERYWHERE,
				DEBUG_MODE_V == DebugMode.FILE
						|| DEBUG_MODE_V == DebugMode.EVERYWHERE,
				createFileNameInCoverageDir("debug.log"),
				createFileNameInCoverageDir("debug"));
	...
	private static String createFileNameInCoverageDir(String name) {
		return RTS_DIR_V + System.getProperty("file.separator") + name;
	}
	...
	private static void printVerbose(File userHome, File userDir) {
		if (DEBUG_MODE_V == DebugMode.SCREEN || DEBUG_MODE_V == DebugMode.FILE
				|| DEBUG_MODE_V == DebugMode.EVERYWHERE) {
			Field[] options = getAllNonHiddenOptions(Config.class);

			printVerboseLine();
			if (userHome != null && userHome.exists()) {
				printVerboseLine("Loaded configuration file from",
			...
			for (Field option : options) {
				String optionName = (String) getValue(
						getNameField(Config.class, option));
				Object value = getValue(option);
				if (value == null) {
					printVerboseLine(optionName, "null");
				} else if (value.getClass().isArray()) {
					printVerboseLine(optionName,
	...
	protected static void loadProperties(Properties props) {
		RTS_DIR_V = getURIString(props, RTS_DIR_N, RTS_DIR_V);
		MODE_V = AgentMode
				.fromString(getString(props, MODE_N, MODE_V.toString()));
		SINGLE_NAME_V = getString(props, SINGLE_NAME_N, SINGLE_NAME_V);
		DEPENDENCIES_FORMAT_V = getString(props, DEPENDENCIES_FORMAT_N,
				DEPENDENCIES_FORMAT_V);
		HASH_ALGORITHM_V = Hasher.Algorithm.fromString(getString(props,
				HASH_ALGORITHM_N, HASH_ALGORITHM_V.toString()));
		DEPENDENCIES_INCLUDE_WELLKNOWN_V = getBoolean(props,
				DEPENDENCIES_INCLUDE_WELLKNOWN_N,
				DEPENDENCIES_INCLUDE_WELLKNOWN_V);
		X_ENABLED_V = getBoolean(props, X_ENABLED_N, X_ENABLED_V);
		X_LOG_RUNS_V = getBoolean(props, X_LOG_RUNS_N, X_LOG_RUNS_V);
		X_INSTRUMENT_CODE_V = getBoolean(props, X_INSTRUMENT_CODE_N,
				X_INSTRUMENT_CODE_V);
		X_DEPENDENCIES_SAVE_V = getBoolean(props, X_DEPENDENCIES_SAVE_N,
				X_DEPENDENCIES_SAVE_V);
		DEBUG_V = getBoolean(props, DEBUG_N, DEBUG_V);
		DEBUG_MODE_V = DebugMode.fromString(
				getString(props, DEBUG_MODE_N, DEBUG_MODE_V.toString()));
		CACHE_SIZES_V = getInteger(props, CACHE_SIZES_N, CACHE_SIZES_V);
		DEPENDENCIES_IGNORED_PATHS_V = getPattern(props,
				DEPENDENCIES_IGNORED_PATHS_N, DEPENDENCIES_IGNORED_PATHS_V);
		DEPENDENCIES_CLASSES_INSTRUMENT_V = getPattern(props,
				DEPENDENCIES_CLASSES_INSTRUMENT_N,
				DEPENDENCIES_CLASSES_INSTRUMENT_V);
		SELECTION_EXCLUDES_V = getArray(props, SELECTION_EXCLUDES_N,
				SELECTION_EXCLUDES_V);
		SELECTION_INCLUDES_V = getArray(props, SELECTION_INCLUDES_N,
				SELECTION_INCLUDES_V);
		FORCE_ALL_V = getBoolean(props, FORCE_ALL_N, FORCE_ALL_V);
		FORCE_FAILING_V = getBoolean(props, FORCE_FAILING_N, FORCE_FAILING_V);
		HASH_WITHOUT_DEBUGINFO_V = getBoolean(props, HASH_WITHOUT_DEBUGINFO_N,
				HASH_WITHOUT_DEBUGINFO_V);
		CACHE_SEEN_CLASSES_V = getBoolean(props, CACHE_SEEN_CLASSES_N,
				CACHE_SEEN_CLASSES_V);
		X_IGNORE_ALL_TESTS_V = getBoolean(props, X_IGNORE_ALL_TESTS_N,
				X_IGNORE_ALL_TESTS_V);
		DEPENDENCIES_APPEND_V = getBoolean(props, DEPENDENCIES_APPEND_N,
				DEPENDENCIES_APPEND_V);
		X_SAVE_INSTRUMENTED_CODE_V = getBoolean(props,
				X_SAVE_INSTRUMENTED_CODE_N, X_SAVE_INSTRUMENTED_CODE_V);
		X_SAVE_HASHER_CACHE_V = getBoolean(props, X_SAVE_HASHER_CACHE_N,
				X_SAVE_HASHER_CACHE_V);
		DEPENDENCIES_NIO_V = getBoolean(props, DEPENDENCIES_NIO_N,
				DEPENDENCIES_NIO_V);
		DEPENDENCIES_NIO_INCLUDES_V = getPattern(props,
				DEPENDENCIES_NIO_INCLUDES_N, DEPENDENCIES_NIO_INCLUDES_V);
		DEPENDENCIES_NIO_EXCLUDES_V = getPattern(props,
				DEPENDENCIES_NIO_EXCLUDES_N, DEPENDENCIES_NIO_EXCLUDES_V);
	}
	...
	/**
	 * This method is used to load path to .rts when the path is given as URI
	 * string. We need to use URI to make sure that we support paths (given to
	 * javaagent) even if they contain spaces.
	 */
	private static String getURIString(Properties props, String key,
			String def) {
		try {
	...
	private static boolean getBoolean(Properties props, String key,
			boolean def) {
		return Boolean
				.parseBoolean(getString(props, key, Boolean.toString(def)));
	}
	...
	protected static Pattern getPattern(Properties props, String key,
			Pattern def) {
	...
	protected static Set<String> getAllOptionNames(Class<?> clz) {
		...
		Field[] options = getAllOptions(clz);
		for (Field option : options) {
			names.add(option.getName().replace("_V", "").toLowerCase()
					.replaceAll("_", "\\."));
		}
		return names;
	}
	...
	private static Field getNameField(Class<?> clz, Field field) {
		...
		String valueName = fieldName.substring(0, fieldName.lastIndexOf("_V"))
				+ "_N";
		try {
		...
		if (packed != null && !packed.equals("")) {
			...
			for (String opt : options) {
				...
				if (keyValue.length != 2)
					throw new RuntimeException("Incorrect argument: " + opt);
	...
	public static Hasher createHasher() {
		return X_SAVE_HASHER_CACHE_V
				? new FileCachingHasher(Config.HASH_ALGORITHM_V,
						Config.CACHE_SIZES_V, Config.HASH_WITHOUT_DEBUGINFO_V,
						new File(Config.RTS_DIR_V, "hasher-cache.txt"))
				: new Hasher(Config.HASH_ALGORITHM_V, Config.CACHE_SIZES_V,
						Config.HASH_WITHOUT_DEBUGINFO_V);
	}

	public static DependencyAnalyzer createDepenencyAnalyzer() {
		return new DependencyAnalyzer(CACHE_SIZES_V, createHasher(),
				createStorer(), SELECTION_EXCLUDES_V, SELECTION_INCLUDES_V);
	}
