...
import org.ijacoco.core.rts.dynamic.DynamicRTS;
...
/**
 * Tool Facade/Singleton. This class should be the only interaction between
 * other code (e.g., JUnit) and Tool. This class should be notified when the
 * recording (of coverage) should start and when the recording should stop;
 * everything else (e.g., instrumentation, hashing, etc.) should be initiated by
 * this class (or from other classes invoked by this class).
 *
...
public final class RTS {
	...
	/**
	 * Returns the only instance of this class. This method will construct and
	 * initialize the instance if it was not previously constructed.
	 */
	public static RTS inst() {
		if (inst != null)
			return inst;
	...
	public boolean isClassAffected(String className) {
		...
		return mIsEnabled;
		// Disable this because we use Maven and this may interfere with
		// iJaCoCo.
		// return !mIsEnabled || mDependencyAnalyzer.isClassAffected(className);
	}
	...
	public void endClassCoverage(String className, boolean isFailOrError) {
		...
		if (isFailOrError) {
			...
			try {
				...
				Log.e("Unable to create file for a failing test: " + className,
						e);
			}
		} else {
			outcomeFile.delete();
	...
	private boolean initAndReportSuccess() {
		...
		if (!isEnabled || !Config.X_INSTRUMENT_CODE_V
				|| isRTSSystemClassLoader()) {
			return isEnabled;
		}

		// Set the agent at runtime if not already set.
		Instrumentation instrumentation = RTSAgent.getInstrumentation();
		if (instrumentation == null) {
			Log.d("Agent has not been set previously");
			instrumentation = DynamicRTS.initAgentAtRuntimeAndReportSuccess();
			if (instrumentation == null) {
				Log.d("No Instrumentation object found; enabling RTS without using any instrumentation");
				return true;
			}
		} else {
	...
	/**
	 * Returns true if current system class loader is {@code RTSClassLoader}.
	 */
	@Research
	private boolean isRTSSystemClassLoader() {
		ClassLoader systemClassLoader = ClassLoader.getSystemClassLoader();
		return systemClassLoader != null
				? systemClassLoader.getClass().getName()
						.equals(Names.RTS_CLASSLOADER_BIN)
				: false;
	}
