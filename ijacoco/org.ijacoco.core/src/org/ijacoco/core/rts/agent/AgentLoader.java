...
public final class AgentLoader {
	...
	private static final String AGENT_LOADER_INIT = AgentLoader.class.getName()
			+ "Initialize";
	 ...
	 */
	public static boolean loadRTSAgent() {
		...
		try {
			...
			if (System.getProperty("java.version").startsWith("9")) {
				throw new RuntimeException(
						"Running with Java 9 requires -Djdk.attach.allowAttachSelf=true");
			}
			return false;
		}
	}
	 ...
	 *             If unexpected behavior
	 */
	private static void attachAgent(Class<?> vmClass, URL agentJarURL)
			throws Exception {
		String pid = getPID();
		...
		getLoadAgentMethod(vmClass).invoke(vm,
				new Object[] { agentAbsolutePath });
	 ...
	 */
	private static Method getAttachMethod(Class<?> vmClass)
	 ...
	 *             If no such method is found
	 */
	private static Method getLoadAgentMethod(Class<?> vmClass)
			throws SecurityException, NoSuchMethodException {
	 ...
	 *             If no such method is found
	 */
	private static Method getDetachMethod(Class<?> vmClass)
			throws SecurityException, NoSuchMethodException {
	 ...
	 */
	private static Class<?> loadVirtualMachine(File toolsJarFile)
			throws Exception {
	...
	private static File findToolsJar() {
		...
		File toolsJarFile = new File(javaHomeFile,
				"lib" + File.separator + TOOLS_JAR_NAME);

		if (!toolsJarFile.exists()) {
			toolsJarFile = new File(System.getenv("java_home"),
					"lib" + File.separator + TOOLS_JAR_NAME);
		}

		if (!toolsJarFile.exists() && javaHomeFile.getAbsolutePath()
				.endsWith(File.separator + "jre")) {
			javaHomeFile = javaHomeFile.getParentFile();
			toolsJarFile = new File(javaHomeFile,
					"lib" + File.separator + TOOLS_JAR_NAME);
		}

		if (!toolsJarFile.exists() && isMac() && javaHomeFile.getAbsolutePath()
	...
	public static void main(String[] args) {
		System.out.println(loadRTSAgent());
	}
}
