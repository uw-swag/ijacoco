...
final class MethodCheck extends AbstractCheck {
	...
	static class TestAbs {
		...
		public TestAbs(boolean isAffected, Set<RegData> regData,
				String fileName, String fileDir, String className) {
		...
		public void setAffected(boolean b) {
			mIsAffected = b;
		}
	...
	public String includeAll(String fileName, String fileDir) {
		...
		if (index == -1) {
			...
			// do precise work.
			Log.w("Currently we do not support files that do not contain class names: "
					+ fileName);
		} else {
			...
			mTests.add(new TestAbs(isAffected,
					mStorer.load(fileDir, className, methodName), fileName,
					fileDir, className));
		}
		return className;
	}
	...
	public void includeAffected(Set<String> affectedClasses) {
		...
		out: for (int i = 0; i < affectedTests.size(); i++) {
			...
			for (int j = 0; j < nonAffectedTests.size(); j++) {
				...
				if (anyDiff) {
					...
					// We remove flag that the test is affected not to include
					// class later.
		...
		for (TestAbs test : affectedTests) {
			if (test.isAffected())
				affectedClasses.add(test.getClassName());
		}
	}
	...
	private boolean checkForDifferences(TestAbs affected, TestAbs nonAffected) {
		for (RegData affectedDatum : affected.getRegData()) {
			for (RegData nonAffectedDatum : nonAffected.getRegData()) {
				if (affectedDatum.getURLExternalForm()
						.equals(nonAffectedDatum.getURLExternalForm())
						&& !affectedDatum.getHash()
								.equals(nonAffectedDatum.getHash())) {
					return true;
				}
			}
		}
	...
	private List<TestAbs> getAffectedTests(List<TestAbs> tests) {
		List<TestAbs> affectedTests = new ArrayList<TestAbs>();
	...
	private List<TestAbs> getNonAffectedTests(List<TestAbs> tests) {
		List<TestAbs> nonAffectedTests = new ArrayList<TestAbs>();
		for (int i = 0; i < tests.size(); i++) {
			TestAbs test = tests.get(i);
			if (!test.isAffected())
				nonAffectedTests.add(test);
		}
