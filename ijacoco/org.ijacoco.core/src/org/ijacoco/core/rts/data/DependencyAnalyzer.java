...
import org.ijacoco.core.rts.monitor.CoverageMonitor;
...
public final class DependencyAnalyzer {
	...
	/**
	 * Cache of test->run mappings; useful if there are Parameterized tests
	 * (note that we limit the size)
	 */
	 ...
	 * Constructor.
	 */
	public DependencyAnalyzer(int cacheSizes, Hasher hasher, Storer storer,
		...
		this.mRootDir = Config.RTS_DIR_V;
		this.mDependenciesAppend = Config.DEPENDENCIES_APPEND_V;

		this.mUrlExternalForm2Modified = new LRUMap<String, Boolean>(
				cacheSizes);
	...
	/**
	 * We identify cases that are currently (almost) impossible to support
	 * purely from Java.
	 *
	 * org.apache.log4j.net.SocketServerTestCase (from Log4j project) test is
	 * run/specified in build.xml. The test runs a Java process and test in
	 * parallel (two VMs). In addition Java process takes as input the number of
	 * tests that would be executed. Of course this would fail in the first
	 * place because of the number of tests. However, that is not the only
	 * problem; Java process is a server that waits for connection, so if no
	 * test is executed this process would just be alive forever. See
	 * http://svn.apache.org/repos/asf/logging/log4j/trunk (revision 1344108)
	 * and file tests/build.xml for details.
	 */
	@Research
	...
	private boolean beginCoverage(String className, String methodName,
			boolean isRecordAffectedOutcome) {
		...
		if (mFullTestName2Rerun.containsKey(fullMethodName)) {
			Set<RegData> regData = mStorer.load(mRootDir, className,
					methodName);
		...
		// If in append mode add urls; we used this append mode when we noticed
		// that
		// some runs give non-deterministic coverage, so we wanted to run the
		// same
	...
	private void endCoverage(String className, String methodName) {
		Map<String, String> hashes = mHasher
				.hashExternalForms(CoverageMonitor.getURLs());
		Set<RegData> regData = new TreeSet<RegData>(
				new RegData.RegComparator());
	...
	@Research
	private void recordTestAffectedOutcome(String fullMethodName,
			boolean isAffected) {
		...
		try {
			...
			pw.println(mRootDir + "." + fullMethodName + " "
					+ (isAffected ? "RUN" : "SKIP"));
			pw.close();
		} catch (Exception ex) {
			ex.printStackTrace();
