...
/**
 * Storing dependencies in text format. This class stores mapping URL(external
 * form)->hash. There is one line for each URL. URL and hash are separated by '
 * _ ' character.
 */
public class TxtStorer extends Storer {
	...
	protected boolean isMagicCorrect(BufferedReader br) throws IOException {
		if (mCheckMagicSequence) {
			...
			return (magicLine != null
					&& magicLine.equals(mMode.getMagicSequence()));
		} else {
			return true;
		}
	}
	...
	@Override
	protected final void extendedSave(FileOutputStream fos,
			Set<RegData> hashes) {
		...
		try {
			...
			for (RegData regDatum : hashes) {
				printLine(state, pw, regDatum.getURLExternalForm(),
						regDatum.getHash());
			}
	 ...
	 */
	protected void printLine(State state, Writer pw, String externalForm,
