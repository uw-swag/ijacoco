...
package org.ijacoco.core.rts.dynamic;
...
public class DynamicRTS {
	...
	private static Instrumentation addClassfileTransformer() throws Exception {
		...
		Class<?> instrumentationFactoryClass = ClassLoader
				.getSystemClassLoader().loadClass(Names.DYNAMIC_AGENT_BIN);
		Method getInstrumentation = instrumentationFactoryClass
		...
		ClassLoader.getSystemClassLoader()
				.loadClass(CoverageMonitor.class.getName());
	 ...
	 */
	private static void setOpenJpaInstrumentation(
			Instrumentation instrumentation) {
		try {
			Class<?> instrumentationFactoryClass = Class.forName(
					"org.apache.openjpa.enhance.InstrumentationFactory");
			Method getInstrumentation = instrumentationFactoryClass.getMethod(
					"setInstrumentation",
	...
	private static void instrumentAllLoadedClasses(
			Instrumentation instrumentation) {
		Class<?>[] classes = instrumentation.getAllLoadedClasses();
		Log.d("Number of classes to instrument: ", classes.length);
		for (Class<?> clz : classes) {
			if (instrumentation.isModifiableClass(clz)
					&& !Types.isRetransformIgnorable(clz)) {
				try {
	...
	private static void setSystemClassLoaderClassPath() throws Exception {
		...
		URL url = RTSAgent.class
				.getResource(RTSAgent.class.getSimpleName() + ".class");
		String origPath = url.getFile().replace("file:", "")
				.replaceAll(".jar!.*", ".jar");
		...
		if (FileUtil.isSecondNewerThanFirst(junitJar, xtsJar)) {
			...
			String[] includePrefixes = { Names.RTS_PACKAGE_BIN };
			String[] excludePrefixes = { RTSAgent.class.getName() };
			isCreated = JarXtractor.extract(junitJar, xtsJar, includePrefixes,
					excludePrefixes);
		}
		...
		if (isCreated) {
			...
			throw new RuntimeException(
					"Could not extract Tool classes in separate jar.");
		}
	}
