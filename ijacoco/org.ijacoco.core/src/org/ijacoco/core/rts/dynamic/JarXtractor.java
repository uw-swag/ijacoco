...
public final class JarXtractor {
	 ...
	 */
	public static boolean extract(File jarFile, File newJar,
			String[] includePrefixes, String[] excludePrefixes)
			throws IOException {
		...
		for (ZipEntry entry = zis.getNextEntry(); entry != null; entry = zis
			...
			String name = entry.getName().replaceAll("/", ".").replace(".class",
					"");
			boolean isStartsWith = startsWith(name, includePrefixes)
					&& !startsWith(name, excludePrefixes);
	 ...
	 */
	private static List<String> extractClassNames(String jarName)
			throws IOException {
		List<String> classes = new LinkedList<String>();
		ZipInputStream orig = new ZipInputStream(new FileInputStream(jarName));
		for (ZipEntry entry = orig.getNextEntry(); entry != null; entry = orig
				.getNextEntry()) {
			String fullName = entry.getName().replaceAll("/", ".")
					.replace(".class", "");
	...
	public static void main(String[] args) throws IOException {
		...
		extract(new File(jarInput),
				new File("/tmp/junit-org.ijacoco-agent.jar"),
				new String[] { Names.RTS_PACKAGE_BIN }, new String[] {});
		for (String name : extractClassNames(
				"/tmp/junit-org.ijacoco-agent.jar")) {
