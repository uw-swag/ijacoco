...
package org.ijacoco.core.rts.instrument;

import org.ijacoco.core.rts.agent.Instr;
...
public final class CoverageMethodVisitor extends MethodVisitor {
	 ...
	 * Constructor.
	 */
	public CoverageMethodVisitor(String className, int classProbeId,
			AtomicInteger probeCounter, int access, String methodName,
			MethodVisitor mv, boolean isNewerThanJava4) {
	...
	public void visitLdcInsn(Object cst) {
		...
		if (cst instanceof Type) {
			...
			if (sort == Type.OBJECT) {
				String className = Types
						.descToInternalName(((Type) cst).getDescriptor());
	...
	public void visitCode() {
		if (isNonPrivateStaticMethod() || isNonPrivateInit()
				|| isStaticBlock()) {
	...
	protected void insertFInvocation(String owner, String name, String desc) {
		...
		mv.visitMethodInsn(Opcodes.INVOKESTATIC, Names.COVERAGE_MONITOR_VM,
				Instr.COVERAGE_MONITOR_FIELD_MNAME, Instr.OBJECT_I_V_DESC,
				false);
	}

	protected void insertReflectionInvocation(String name, String desc) {
		mv.visitMethodInsn(Opcodes.INVOKESTATIC,
				Instr.REFLECTION_MONITOR_CLASS_INTERNAL_NAME, name,
	...
	private void insertTInvocation0(String className, int probeId) {
		...
		Log.d("XXX CoverageMethodVisitor.insertTInvocation0: className="
				+ className + ", probeId=" + probeId + ", isNonInnerClass="
				+ isNonInnerClass + ", mIsNewerThanJava4=" + mIsNewerThanJava4);
	...
	protected boolean isNonPrivateInit() {
		...
		return /* (mAccess & Opcodes.ACC_PRIVATE) == 0 && */ mMethodName
				.equals("<init>");
	}
	...
	protected boolean isNonPrivateStaticMethod() {
		return (mAccess & Opcodes.ACC_PRIVATE) == 0
				&& (mAccess & Opcodes.ACC_STATIC) != 0;
	}
