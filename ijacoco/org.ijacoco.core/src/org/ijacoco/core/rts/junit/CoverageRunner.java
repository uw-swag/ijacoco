...
public class CoverageRunner extends Runner implements Filterable, Sortable {
	...
	public void run(RunNotifier notifier) {
		if (isIgnoreAllTests()) {
			...
			TimeLog.log("test:" + this.hashCode() + ":beg");
			RTS.inst().beginClassCoverage(mClz.getName());
			JUnit4OutcomeListener rtsListener = new JUnit4OutcomeListener();
			notifier.addListener(rtsListener);
			try {
				mWrappedRunner.run(notifier);
			} finally {
				// Include URLs from constructors.
				if (mURLs != null)
					CoverageMonitor.addURLs(mURLs);
				RTS.inst().endClassCoverage(mClz.getName(),
						rtsListener.isFailOrError());
				TimeLog.log("test:" + this.hashCode() + ":end");
			}
		}
	}
	 ...
	 * the coverage should not be collected in case if wrapped runner satisfies
	 * one of the following conditions: 1) it is our affecting runner, 2) test
	 * suite (Suite.class) runner with several classes; however, we do want to
	 * collect coverage for Parametrized runners (which are subclasses of Suite
	 * runner).
	 *
	...
	private boolean isRunWithoutCoverage() {
		return (mWrappedRunner instanceof AffectingRunner)
				|| (mWrappedRunner instanceof Suite
						&& !(mWrappedRunner instanceof Parameterized
								|| mWrappedRunner instanceof Enclosed));
	 ...
	 * be out). Implementing this method was inspired by ant and "ant
	 * junit-single-test
	 * -Djunit.testcase=org.apache.tools.ant.taskdefs.optional.junit.JUnitTaskTest"
