...
public class JUnit4Monitor {
	...
	/*
	 * Depth of calls in this class to support Suite.class. Unfortunately it is
	 * static now, but we may want to do ThreadLocal eventually.
	 */
	private static int recursiveDepth = 0;

	public static Runner runnerForClass(RunnerBuilder builder,
	 ...
	 * JUnit4 support.
	 */
	public static Runner runnerForClass0(RunnerBuilder builder,
			Class<?> testClass) throws Throwable {
		if (recursiveDepth > 1
				|| isOnStack(0, CoverageRunner.class.getCanonicalName())) {
			return builder.runnerForClass(testClass);
		}
		// We disable AffectingBuilder because RTS already set the excluded test
		// classes via the
		// excludes field.
		...
		return new CoverageRunner(testClass, wrapped,
				CoverageMonitor.getURLs());
	}
