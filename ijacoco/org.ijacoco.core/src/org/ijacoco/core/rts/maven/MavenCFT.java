...
public final class MavenCFT implements ClassFileTransformer {
	...
	private static class MavenMethodVisitor extends MethodVisitor {
		...
		public MavenMethodVisitor(String interceptorName, String methodName,
				String methodDesc, MethodVisitor mv) {
		...
		public void visitCode() {
			...
			mv.visitMethodInsn(Opcodes.INVOKESTATIC, mInterceptorName,
					mMethodName,
	...
	private static class MavenClassVisitor extends ClassVisitor {
		 ...
		 * Constructor.
		 */
		public MavenClassVisitor(String className, String interceptorName,
				ClassVisitor cv) {
		...
		public MethodVisitor visitMethod(int access, String name, String desc,
			...
			MethodVisitor mv = cv.visitMethod(access, name, desc, signature,
					exceptions);
			if (name.equals(MavenNames.EXECUTE_MNAME)
					&& desc.equals(MavenNames.EXECUTE_MDESC)) {
	...
	public byte[] transform(ClassLoader loader, String className,
		...
		if (className.equals(MavenNames.ABSTRACT_SUREFIRE_MOJO_VM)
			...
			return addInterceptor(className, classfileBuffer,
					MavenNames.SUREFIRE_INTERCEPTOR_CLASS_VM);
		} else if (className.equals(MavenNames.TESTMOJO_VM)) {
			return addInterceptor(className, classfileBuffer,
					MavenNames.SCALATEST_INTERCEPTOR_CLASS_VM);
		} else {
			return null;
		}
	}
	...
	private byte[] addInterceptor(String className, byte[] classfileBuffer,
			String interceptorName) {
		ClassReader classReader = new ClassReader(classfileBuffer);
		ClassWriter classWriter = new ClassWriter(classReader,
				ClassWriter.COMPUTE_MAXS);
		ClassVisitor visitor = new MavenClassVisitor(className, interceptorName,
				classWriter);
