 ...
 * configuration is supported by RTS, and also to configure RTS agent.
 */
public class ScalaTestMojoInterceptor extends AbstractMojoInterceptor {
	...
	public static void execute(Object mojo) throws Exception {
		...
		try {
			...
			throwMojoExecutionException(mojo, "Usupported ScalaTest version",
					ex);
		}
	...
	private static void checkVersion(Object mojo) throws Exception {
		try {
			...
			throwMojoExecutionException(mojo,
					"Unsupported ScalaTest org.ijacoco.maven plugin version.",
					ex);
		}
	}
	...
	private static void checkConfiguration(Object mojo) throws Exception {
		...
		if (forkMode == null || !forkMode.equals("once")) {
			throwMojoExecutionException(mojo,
					"Fork has to be enabled when running tests with RTS; check forkMode parameter.",
					null);
		}
	}
	...
	private static void updateParallel(Object mojo) throws Exception {
		...
		if (currentParallel) {
			warn(mojo,
					"RTS does not support parallel parameter.  This parameter will be set to false for this run.");
	...
	private static void updateArgLine(Object mojo) throws Exception {
		...
		// Note that ScalaTest org.ijacoco.maven plugin does not support
		// fork option at this point.
		String newArgLine = makeArgLine(mojo, Config.AgentMode.SCALATEST,
				currentArgLine);
