...
/**
 * Monitor invoked before Surefire. The purpose of monitoring is to adjust the
 * arguments for Surefire to integrate RTS at appropriate places.
 */
public final class SurefireMojoInterceptor extends AbstractMojoInterceptor {
	...
	private static final String EXIT_TIMEOUT_FIELD = "forkedProcessExitTimeoutInSeconds";
	// Set default exit timeout to 300 seconds so that the shutdown hook will be
	// alive for at least
	// 300 seconds.
	private static final int DEFAULT_EXIT_TIMEOUT = 300;
	...
	private static void checkSurefireVersion(Object mojo) throws Exception {
		try {
			...
			getField(EXIT_TIMEOUT_FIELD, mojo);
		} catch (NoSuchMethodException ex) {
			throwMojoExecutionException(mojo,
					"Unsupported surefire version.  An alternative is to use select/restore goals.",
					ex);
		}
	}

	/**
	 * Checks that surefire configuration allows integration with RTS. At the
	 * moment, we check that forking is enabled.
	 */
	private static void checkSurefireConfiguration(Object mojo)
			throws Exception {
		...
		if ((forkCount != null && forkCount.equals("0")) || (forkMode != null
				&& (forkMode.equals("never") || forkMode.equals("none")))) {
			throwMojoExecutionException(mojo,
					"Fork has to be enabled when running tests with RTS; check forkMode and forkCount parameters.",
					null);
		}
	}

	private static void updateArgLine(Object mojo) throws Exception {
		Config.AgentMode junitMode = isOneVMPerClass(mojo)
				? Config.AgentMode.JUNITFORK
	...
	private static void updateExcludes(Object mojo) throws Exception {
		...
		List<String> rtsExcludes = new ArrayList<String>(
				Arrays.asList(System.getProperty(EXCLUDES_INTERNAL_PROP)
						.replace("[", "").replace("]", "").split(",")));
		List<String> newExcludes = rtsExcludes;
		if (currentExcludes != null) {
			newExcludes.addAll(currentExcludes);
		} else {
			// Add default excludes as specified by Surefire if excludes is not
	...
	private static void updateParallel(Object mojo) throws Exception {
		try {
			...
			if (currentParallel != null) {
				warn(mojo,
						"RTS does not support parallel parameter.  This parameter will be set to null for this run.");
	...
	private static void updateExitTimeout(Object mojo) throws Exception {
		setField(EXIT_TIMEOUT_FIELD, mojo, DEFAULT_EXIT_TIMEOUT);
	}
