...
public final class CoverageMonitor {
	...
	private static final String JUNIT_FRAMEWORK_URL_PART = "!/"
			+ Names.JUNIT_FRAMEWORK_PACKAGE_VM;
	private static final String ORG_JUNIT_URL_PART = "!/"
			+ Names.ORG_JUNIT_PACKAGE_VM;
	private static final String ORG_HAMCREST_URL_PART = "!/"
			+ Names.ORG_HAMCREST_VM;
	private static final String ORG_APACHE_MAVEN_URL_PART = "!/"
			+ Names.ORG_APACHE_MAVEN_VM;
	...
	public static void t(Class<?> clz) {
		...
		Log.d("!!! CoverageMonitor.t: " + clz.getName());

		// Check and assign id to this class (this must be synchronized).
		try {
			Log.d("!!! CoverageMonitor.t: " + clz.getName() + ", getting lock");
			sLock.lock();
			Log.d("!!! CoverageMonitor.t: " + clz.getName() + ", got lock");
			if (!sClasses.add(clz)) {
				return;
			}
		} finally {
		...
		String resourceName = className
				.substring(className.lastIndexOf(".") + 1).concat(".class");
		URL url = null;
		try {
	...
	private static void safeRecordURL(String externalForm) {
		try {
			Log.d("!!! CoverageMonitor.safeRecordURL: " + externalForm
					+ ", getting lock");
			sLock.lock();
			Log.d("!!! CoverageMonitor.safeRecordURL: " + externalForm
					+ ", got lock");
	...
	private static boolean filterFile(String absolutePath) {
		...
		return (absolutePath.contains(Config.RTS_DIR_V)
				|| absolutePath.contains("/" + Names.RTS_ROOT_DIR_NAME)
	...
	private static boolean filterURL(String externalForm) {
		...
		if (IS_UNIX && (externalForm.startsWith(TMP_FILE_PREFIX)
				|| externalForm.startsWith(TMP_JAR_PREFIX))) {
			return true;
		}
		// Ignore paths that are specified in configuration.
		if (Config.DEPENDENCIES_IGNORED_PATHS_V != null) {
			if (Config.DEPENDENCIES_IGNORED_PATHS_V.matcher(externalForm)
					.find()) {
				return true;
			}
		}
