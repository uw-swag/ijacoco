...
package org.ijacoco.core.rts.monitor;
...
import org.ijacoco.core.rts.instrument.CoverageMethodVisitor;
import org.ijacoco.core.rts.util.Types;

/**
 * Dependency monitor for reflection methods. Namely, each reflection method is
 * replaced by the invocation to this class (see {@link CoverageMethodVisitor}).
 * The instrumented version of each method records covered classes and invokes
 * the original reflection method.
 *
...
public final class ReflectionMonitor {
	...
	public static Constructor<?> getConstructor(Class<?> clz,
			Class<?>... parameterTypes)
			throws SecurityException, NoSuchMethodException {
		recordClass(clz);
	...
	public static Method getDeclaredMethod(Class<?> clz, String name,
			Class<?>... parameterTypes)
			throws SecurityException, NoSuchMethodException {
		recordClass(clz);
	...
	public static <A extends Annotation> A getAnnotation(Class<?> clz,
			Class<A> annotationClass) {
		recordClass(clz);
		return clz.getAnnotation(annotationClass);
	}

	public static boolean isAnnotationPresent(Class<?> clz,
	...
	public static <A extends Annotation> A[] getAnnotationsByType(Class<?> clz,
			Class<A> annotationClass) throws Exception {
		recordClass(clz);
		Method m = Class.class.getDeclaredMethod("getAnnotationsByType",
				Class.class);
	...
	public static <A extends Annotation> A getDeclaredAnnotation(Class<?> clz,
			Class<A> annotationClass) throws Exception {
		recordClass(clz);
		Method m = Class.class.getDeclaredMethod("getDeclaredAnnotation",
				Class.class);
		return (A) m.invoke(clz, annotationClass);
		// when we move to Java 8:
	...
	public static <A extends Annotation> A[] getDeclaredAnnotationsByType(
			Class<?> clz, Class<A> annotationClass) throws Exception {
		recordClass(clz);
		Method m = Class.class.getDeclaredMethod("getDeclaredAnnotationsByType",
				Class.class);
		return (A[]) m.invoke(clz, annotationClass);
		// when we move to Java 8:
