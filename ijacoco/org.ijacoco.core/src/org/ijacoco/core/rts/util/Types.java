...
public final class Types {
	...
	private static final String ORG_XML_SAX_BIN = internalToBinName(
			ORG_XML_SAX_VM);

	private static final String ORG_IETF_JGSS_VM = "org/ietf/jgss/";
	private static final String ORG_IETF_JGSS_BIN = internalToBinName(
			ORG_IETF_JGSS_VM);
	...
	private static final String ORG_MOCKITO_BIN = internalToBinName(
			ORG_MOCKITO_VM);

	private static final String ORG_JACOCO_AGENT_VM = "org/jacoco/agent/";
	private static final String ORG_JACOCO_AGENT_BIN = internalToBinName(
			ORG_JACOCO_AGENT_VM);

	private static final String iJACOCO_VM = Names.iJACOCO_PACKAGE_VM
			.concat("/");
	private static final String iJACOCO_BIN = internalToBinName(iJACOCO_VM);

	/**
	 * Checks if class should be ignored when collecting dependencies.
	 */
	public static boolean isIgnorable(Class<?> clz) {
		return clz.isArray() || clz.isPrimitive()
				|| isIgnorableBinName(clz.getName());
	}
	...
	public static boolean isIgnorableInternalName(String className) {
		return (className.startsWith("[", 0) || className.startsWith(JAVA_VM, 0)
				...
				|| className.startsWith(iJACOCO_VM, 0)
	...
	public static boolean isIgnorableBinName(String className) {
		return (className.startsWith("[", 0)
				...
				|| className.startsWith(iJACOCO_BIN, 0)
	...
	public static URL getResource(Class<?> clz) {
		return clz
				.getResource("/" + clz.getName().replace('.', '/') + ".class");
	}
